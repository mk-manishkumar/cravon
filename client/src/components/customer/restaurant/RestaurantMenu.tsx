...
interface RestaurantMenuProps {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  handleAddToCart: (menuItem: any, quantity?: number) => void;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function MenuItemCard({ item, handleAddToCart }: Readonly<{ item: any; handleAddToCart: (item: any, quantity?: number) => void }>) {
    ...
    <div className="flex justify-between items-stretch p-6 bg-white border border-gray-100 shadow-sm rounded-2xl hover:shadow-md transition-shadow">
      <div className="flex-1 pr-6 flex flex-col justify-between">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <button type="button" onClick={onAdd} className="cursor-pointer h-9 px-6 bg-[#FF7A30] hover:bg-[#FF8E4D] text-white font-bold rounded-xl shadow-[0_4px_12px_rgba(255,122,48,0.3)] transition-transform active:scale-[0.98] text-sm uppercase">
            ADD
          </button>
        </div>
      </div>
