...
export default function DesktopAuthDropdown() {
  const user = useAuthStore((state) => state.user);
  const logout = useAuthStore((state) => state.logout);
  const isLoading = useAuthStore((state) => state.isLoading);
  const [showDropdown, setShowDropdown] = useState(false);

  if (isLoading) return <div className="hidden md:block w-10 h-10"></div>; // Placeholder space
        ...
        <div className="absolute right-0 top-full pt-2 w-56 z-50">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2">
            ...
            <div className="py-2">
              ...
              <Link href="/orders" className="flex items-center gap-3 px-5 py-2.5 text-[14px] text-gray-700 hover:bg-[#FFFBF8] hover:text-[#FF3D57] transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M16 16h6" /><path d="M19 13v6" /><path d="M21 10H3" /><path d="M16 2l5 5" /><path d="M3 10v9a2 2 0 0 0 2 2h6" /><path d="M3 10l5-8h8" />
                </svg>
                My Orders
              </Link>
              <Link href="/account" className="flex items-center gap-3 px-5 py-2.5 text-[14px] text-gray-700 hover:bg-[#FFFBF8] hover:text-[#FF3D57] transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
